#!/usr/bin/env python3
"""
Mac X Menos Catalog Updater
- Downloads Tarifas_Lois.xlsx every 1.5 hours via GitHub Actions
- Filters Apple products, applies Mac X Menos pricing formula:
    precio_final = precio_coste * (1 - dto/100) * 1.045
...
# ── Mac X Menos price formula ──
# 1. precio_coste * (1 - dto/100)  → apply discount
# 2. * 1.045                       → add 4.5%
# canon is stored separately and shown in label
def calc_price(precio, dto, canon):
    return round(precio * (1 - dto / 100) * 1.045, 2)
...
def build_products(apple_df):
    ...
    for _, r in apple_df.iterrows():
        ...
        dto    = float(r['dto'])
        canon  = float(r['canon'])
        price  = calc_price(precio, dto, canon)
